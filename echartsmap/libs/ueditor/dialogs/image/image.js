...
(function () {
                    ...
                    if (browser.ie && browser.version <= 7) {
                        ...
                        uploader.makeThumb(file, function (error, src) {
                            if (error || !src) {
                                ...
                                // mengcy 插入图片说明输入框
                                var $input = $('<textarea placeholder="图片描述">');
                                $wrap.append($input);
            ...
            uploader.on('uploadSuccess', function (file, ret) {
                ...
                try {
                    ...
                    if (json.state == 'SUCCESS') {
                        // mengcy 获取图片描述
                        var $text = $file.find('textarea');
                        json.description = $text.val();
    ...
    /* 在线图片，整合图片库修改 by mengcy 2017-10-11 */
    ...
    OnlineImage.prototype = {
        ...
        /** 初始化 */
        init: function () {
            var url = '../../../../website/article/form/picture-lib.html';
            $('#picFrame').attr('src',url);
        },

        getInsertList: function () {
            var list = [];

            var groupId = $("#picFrame")[0].contentWindow.app.currentSelPic.picGroupId;
            var catalogId = $("#picFrame")[0].contentWindow.app.currentSelPic.catalogId;

            if(groupId){
                // 获取组内所有图片
                $.ajax({
                    url: '../../../../../main/cms/picture/getPicList.do',
                    dataType: 'json',
                    async: false,
                    data: {'picGroupId': groupId, 'catalogId': catalogId},
                    error: function () {
                        alert("请求组图数据错误。");
                    },
