...
(function(){
    ...
    function initTabs(){
        ...
        for (var i = 0; i < tabs.length; i++) {
            domUtils.on(tabs[i], "click", function (e) {
                ...
                if(target.getAttribute('data-content-id') == 'video-lib'){
    ...
    function createHtml(url) {
        var iframe = $(url);
        var p = $('<p class="app-video-container"></p>');
        iframe.addClass('app-video-iFrame');
    ...
    function insertUpload(){
        var videoObjs=[],
            ...
            width = 0;//由css样式app-video统一控制视频样式 parseInt($G('upload_width').value, 10) || '480',
            height = 0;//由css样式app-video统一控制视频样式 parseInt($G('upload_height').value, 10) || '',
            align = "";//由css样式app-video统一控制视频样式 findFocus("upload_alignment","name") || 'center';
    ...
    VideoLib.prototype = {
         ...
         */
        init:function () {
            var url = '../../../../website/article/form/video-lib.html';
        ...
        insertSelect: function () {
            ...
            var width = 0;//由css样式app-video统一控制视频样式 parseInt($G('upload_width').value, 10) || 480,
                height = 0;//由css样式app-video统一控制视频样式 parseInt($G('upload_height').value, 10) || '',
                align = '';

            var videoObjs = [];
            var selectedVideo = $("#videoFrame")[0].contentWindow.app.getSelectedVideos();
            for (var i in selectedVideo) {
                videoObjs.push({
                    url : selectedVideo[i].url,
                    poster : selectedVideo[i].coverPath,
                    width : width,
                    height : height,
                    align : align,
                    content : selectedVideo[i].description
                });

                // videoObjs.push({
                //     src: selectedVideo[i].url,
                //     _src: selectedVideo[i].url,
                //     content: selectedVideo[i].title,
                //     poster: selectedVideo[i].coverPath,
                //     floatStyle: "center",
                //     style: "max-width: 100%;"
                // })
            }
            // alert(JSON.stringify(videoObjs));
            editor.execCommand('insertvideo', videoObjs, 'upload');
        }
    };
